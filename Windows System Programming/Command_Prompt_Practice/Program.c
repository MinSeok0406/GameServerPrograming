...
int CmdProcessing(int tokenNum)
{
    ...
    else if (!_tcscmp(cmdTokenList[0], _T("start")))
    {
        if (tokenNum > 1)
        {
            // cmd 형태로 프로세스를 생성 시에는 /k 혹은 /c 을 붙여야 문자열이 버퍼에 저장된다
            _tcscat(optString, _T("/k"));
            for (int i = 1; i < tokenNum; i++)
            {
                _stprintf(optString, _T("%s %s"), optString, cmdTokenList[i]);
                _stprintf(cmdStringWithOptions, _T("%s %s"), _T("cmd.exe"), optString);
            }
        }
        else
        {
            _stprintf(cmdStringWithOptions, _T("%s"), _T("cmd.exe"));
        }
    ...
    else if (!_tcscmp(cmdTokenList[0], _T("kp")))
    {
        ...
        // cmdTokenList[1] 값만 써서 사용해도 됨
        // 이 반복문은 없어도 됨
        _stprintf(optString, _T("%s %s"), optString, cmdTokenList[1]);
        for (int i = 0; i < _tcslen(optString); i++)
        {
            if (optString[i] == '\0')
            {
                break;
            }
            optString[i] = optString[i + 1];
        }

        TCHAR* command;
        do
        {
            command = StrLower(pe32.szExeFile);
            if (!_tcscmp(command, optString))
            {
                break;
            }
        } while (Process32Next(hProcessSnap, &pe32));
        ...
        if (_tcscmp(command, optString))
        {
            _tprintf(_T("command name error! \n"));
            CloseHandle(hProcessSnap);
            return -1;
        }
        ...
        DWORD return_val;
        if (GetExitCodeProcess(process, &return_val) == 0)
        {
            _tprintf(_T("GetExitCodeProcess error! \n"));
            CloseHandle(hProcessSnap);
            return -1;
        }

        if (TerminateProcess(process, return_val) == 0)
        {
            _tprintf(_T("TerminateProcess error! \n"));
            CloseHandle(hProcessSnap);
            return -1;
        }
    ...
    else if (!_tcscmp(cmdTokenList[0], _T("sort")))
    {
        ...
        {
            isRun = CreateProcess(NULL, cmdTokenList[0], NULL, NULL, FALSE,
                0, NULL, NULL, &si, &pi);
...
void TypeTextFile()
{
    ...
    if (!_tcscmp(cmdTokenList[2], _T("|")))
    {
        ...
        isRun = CreateProcess(NULL, cmdTokenList[3], NULL, NULL,
