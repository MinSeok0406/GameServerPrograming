...
const wchar_t* SERVERIP = L"procademyserver.iptime.org";
...
int wmain(int argc, WCHAR* argv[])
{
    ...
    auto flag = DomainToIP(SERVERIP, &sockaddrin.sin_addr);
    if (flag == FALSE)
    {
        wprintf(L"도메인 IP 변환 실패\n");
        return 1;
    }
    ...
    retval = send(sock, (char*)&stPH, sizeof(stPH), 0);
    if (retval == SOCKET_ERROR)
    {
        wprintf(L"%d\n", WSAGetLastError());
        return 1;
    ...
    wchar_t recvMessage[BUFSIZE];
    retval = recv(sock, (char*)recvMessage, BUFSIZE, MSG_WAITALL);
    if (retval == SOCKET_ERROR)
    {
        wprintf(L"%d\n", WSAGetLastError());
        return 1;
